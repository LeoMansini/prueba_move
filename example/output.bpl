...
// fun step_1::sum [verification] at ./sources/example.move:2:5+68
procedure {:timeLimit 40} $2_step_1_sum$verify(_$t0: int, _$t1: int) returns ($ret0: int)
{
    ...
    // assume WellFormed($t0) at ./sources/example.move:2:5+1
    assume {:print "$at(2,29,30)"} true;
    assume $IsValid'u64'($t0);

    // assume WellFormed($t1) at ./sources/example.move:2:5+1
    assume $IsValid'u64'($t1);

    // trace_local[first]($t0) at ./sources/example.move:2:5+1
    assume {:print "$track_local(0,0,0):", $t0} $t0 == $t0;

    // trace_local[second]($t1) at ./sources/example.move:2:5+1
    assume {:print "$track_local(0,0,1):", $t1} $t1 == $t1;

    // $t2 := +($t0, $t1) on_abort goto L2 with $t3 at ./sources/example.move:3:15+1
    ...
    // trace_return[0]($t2) at ./sources/example.move:3:9+14
    assume {:print "$track_return(0,0,0):", $t2} $t2 == $t2;

    // label L1 at ./sources/example.move:4:5+1
    assume {:print "$at(2,96,97)"} true;
L1:

    // assert Not(Gt(Add($t0, $t1), 18446744073709551615)) at ./sources/example.move:7:9+35
    assume {:print "$at(2,122,157)"} true;
    assert {:msg "assert_failed(2,122,157): function does not abort under this condition"}
      !(($t0 + $t1) > 18446744073709551615);

    // assert Eq<u64>($t2, Add($t0, $t1)) at ./sources/example.move:8:9+33
    assume {:print "$at(2,166,199)"} true;
    assert {:msg "assert_failed(2,166,199): post-condition does not hold"}
      $IsEqual'u64'($t2, ($t0 + $t1));

    // return $t2 at ./sources/example.move:8:9+33
    $ret0 := $t2;
    return;

    // label L2 at ./sources/example.move:4:5+1
    assume {:print "$at(2,96,97)"} true;
L2:

    // assert Gt(Add($t0, $t1), 18446744073709551615) at ./sources/example.move:6:5+102
    assume {:print "$at(2,103,205)"} true;
    assert {:msg "assert_failed(2,103,205): abort not covered by any of the `aborts_if` clauses"}
      (($t0 + $t1) > 18446744073709551615);

    // abort($t3) at ./sources/example.move:6:5+102
    $abort_code := $t3;
    $abort_flag := true;
    return;
